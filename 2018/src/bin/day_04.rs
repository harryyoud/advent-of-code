...
fn part_1(sleeps: &HashMap<usize, Vec<SleepEvent>>) -> usize {
    let (guard_num, events) = sleeps
        ...
        .max_by_key(|(_guard_num, v)| {
            v.iter().map(|x| {
                (x.hour_range.clone().count() * 60) + (x.minute_range.clone().count())
            }).sum::<usize>()
        })
        .unwrap();


    let most_common_minute = events.iter()
...
fn part_2(sleeps: &HashMap<usize, Vec<SleepEvent>>) -> usize {
    let (guard_num, asleep_max_minute) = sleeps.iter()
        .map(|(guard_num, events)| {
            (*guard_num, events.iter()
                .flat_map(|x| x.minute_range.clone())
                .counts()
                .into_iter()
                .max_by_key(|(_minute, count)| *count)
...
fn parse_input(input: &str) -> HashMap<usize, Vec<SleepEvent>> {
    ...
    for line in input.lines().sorted() {
        let re = regex!(r#"^\[(?<year>\d{4})-(?<month>\d{2})-(?<day>\d{2}) (?<hour>\d{2}):(?<minute>\d{2})\] (?:Guard #(?<guard_num>\d+) begins shift|falls asleep|wakes up)$"#);
        let capture = re.captures(line).unwrap();

        if line.contains("begins shift") {
            current_guard = capture["guard_num"].parse().unwrap();
        } else if line.contains("falls asleep") {
            sleep_start = (capture["hour"].parse().unwrap(), capture["minute"].parse().unwrap());
        } else if line.contains("wakes up") {
            guard_sleeps.entry(current_guard).or_default().push(SleepEvent {
