use std::{collections::HashMap, ops::{BitAnd, BitOr, Not, Shl, Shr}};
...
fn calculate_input_recursive(cache: &mut HashMap<String, u32>, circuit: &HashMap<String, Input>, target: &str) -> u32 {
...
fn parse_input(input: &str) -> HashMap<String, Input> {
    ...
    for line in input.lines() {
        ...
        if lhs.starts_with("NOT ") {
            ...
            wires.insert(rhs.to_string(), Input::Operand(Operand::And(
                parse_wire(and_left),
                parse_wire(and_right),
            )));
        } else if lhs.contains(" OR ") {
            let (and_left, and_right) = lhs.split(" OR ").collect_tuple().unwrap();            
            wires.insert(rhs.to_string(), Input::Operand(Operand::Or(
                parse_wire(and_left),
                parse_wire(and_right),
            )));
        } else if lhs.contains(" LSHIFT ") {
            let (lshift_left, lshift_right) = lhs.split(" LSHIFT ").collect_tuple().unwrap();
            wires.insert(rhs.to_string(), Input::Operand(Operand::LeftShift(
            ...
            wires.insert(rhs.to_string(), Input::Operand(Operand::RightShift(
