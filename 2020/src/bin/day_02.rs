...
fn part_1(input: &[(Policy, &str)]) -> usize {
    input.into_iter()
        .filter(|(policy, password)| policy.is_valid_v1(password)).count()
}

fn part_2(input: &[(Policy, &str)]) -> usize {
    input.into_iter()
        .filter(|(policy, password)| policy.is_valid_v2(password)).count()
}
...
impl Policy {
    fn is_valid_v1(&self, password: &str) -> bool {
        (self.left..=self.right).contains(
            &password.chars().filter(|x| *x == self.character).count()
        )
    }
...
fn parse_line(line: &str) -> (Policy, &str) {
    let (_, lower, upper, c, password) = regex_captures!(
        r#"(\d+)-(\d+) ([a-z]): ([a-z]+)"#,
        line
